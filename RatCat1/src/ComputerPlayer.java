...
public class ComputerPlayer extends Player{
	...
	*/
	public boolean decideDrawCard(Card discard){
		...
		if(!discard.isPowerCard()){
			for(int i=0;i<4;i++){
				if(knownCards[i] == KNOWN){
					if(playerHand.getCard(i).isPowerCard()){
						if(discard.getRank() < POWER_VALUE){
							take = true;
						}
					}else if(discard.getRank() < playerHand.getCard(i).getRank()){
						take = true;
					}
	...
	*/
	public Card playCard(Card drawnCard){
		...
		if(!drawnCard.isPowerCard()){
			...
			if(index != 5){
				discardCard = playerHand.swapCard(index, drawnCard);
	...
	*/
	public int[] playSwap(){
		...
		for (int i =0; i<4 ;i++) {
			if(knownCards[i]==KNOWN && !playerHand.getCard(i).isPowerCard()){
				if(playerHand.getCard(i).getRank() > highestValue){
					highestValue = playerHand.getCard(i).getRank();
					computerIndex = i;
				}
			}	
		}
		...
		for(int i = 0 ; i<4; i++){
			...
			if(knownCards[i] == KNOWN){
				if(playerHand.getCard(i).isPowerCard()){
					...
					int tempSwapValue = playerHand.getCard(i).getRank() - rank;
					if(tempSwapValue > swapValue){
						swapValue = tempSwapValue;
						index = i;
					}
		...
		for(int i = 0;i<4;i++){
			if(knownCards[i] == KNOWN){
				if(playerHand.getCard(i).isPowerCard()){
					...
					sum += playerHand.getCard(i).getRank();
				}
			}else{
				sum += UNKNOWN_VALUE;
			}
